        ...
        with self.g.as_default():
            ...
            with tf.name_scope('Layers'):
                ...
                with tf.name_scope("dropout4"):
                    dropout4 = tf.layers.dropout(inputs=h4, rate=param.dropoutRate)
                ...
                with tf.name_scope("dropout5"):
                    dropout5 = tf.layers.dropout(inputs=h5, rate=param.dropoutRate)

                with tf.name_scope('Outputs'):
                    Y1 = tf.layers.dense(inputs=dropout5, units=self.outputShape1[0], activation=tf.nn.sigmoid)
                    Y2 = tf.layers.dense(inputs=dropout5, units=self.outputShape2[0], activation=tf.nn.elu)
                    Y3 = tf.nn.softmax(Y2)
                    self.Y1 = Y1
                    self.Y3 = Y3
